...
import { getGetMyContractorProfileQueryKey, useGetMyContractorProfile, useUpsertMyContractorProfile } from '@workspace/api-client-react';
...
export default function ContractorProfileScreen() {
  ...
  const upsert = useUpsertMyContractorProfile({ mutation: {
    onSuccess: (result) => Alert.alert(profile.data ? (isArabic ? 'تم تحديث الملف' : 'Profile updated') : (isArabic ? 'تم إنشاء التجربة' : 'Trial created'), isArabic ? `ينتهي اشتراكك التجريبي في ${new Date(result.subscription.trialEndsAt).toLocaleDateString()}.` : `Your subscription trial runs until ${new Date(result.subscription.trialEndsAt).toLocaleDateString()}.`, [{ text: isArabic ? 'عرض الاشتراك' : 'View subscription', onPress: () => router.replace('/subscription' as never) }]),
