          ...
          {visibleMode !== 'Properties' ? <View style={[styles.budgetCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
           ...
           {visibleMode === 'Providers' ? (
            <View>
                ...
                {(!directory.isLoading || directory.data) ? filteredProviders.map((provider) => <ProviderCard key={provider.id} image={provider.image ?? require('@/assets/images/contractor-project.jpg')} name={isArabic ? provider.nameAr : provider.name} specialty={isArabic ? provider.specialtyAr : provider.specialty} rating={provider.rating} reviews={provider.reviews} distance={provider.distance} verified={provider.verified} saved={savedIds.includes(provider.id)} onPress={() => router.push({ pathname: '/provider/[id]', params: { id: provider.id } })} onSave={() => toggleSaved(provider.id, { subjectKind: 'provider', subjectName: isArabic ? provider.nameAr : provider.name })} />) : null}
            ...
            <View>
               ...
               <View style={styles.propertyGrid}>{filteredListings.map((listing) => <PropertyCard key={listing.id} listing={{ ...listing, title: isArabic ? listing.titleAr : listing.title, location: isArabic ? listing.locationAr : listing.location, type: isArabic ? listing.typeAr : listing.type }} saved={savedIds.includes(listing.id)} onPress={() => router.push({ pathname: '/listing/[id]', params: { id: listing.id } })} onSave={() => toggleSaved(listing.id, { subjectKind: 'property', subjectName: isArabic ? listing.titleAr : listing.title })} />)}</View>
